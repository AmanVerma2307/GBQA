...
y_train_id = np.load('./Datasets/y_train_id_GBQA-CU-5050_Soli.npz',allow_pickle=True)['arr_0']
y_dev_id = np.load('./Datasets/y_dev_id_GBQA-CU-5050_Soli.npz',allow_pickle=True)['arr_0']

y_dev_id = y_dev_id - 5 # Scaling the labels into 0-5 range
...
parser.add_argument('--lambda_id',
                    type=float,
                    help='Scaling weights for the ID loss')
...
dense_hgr = tf.keras.layers.Dense(6,activation='softmax')(dense1)
dense_id = tf.keras.layers.Dense(5,activation='softmax')(dense1)

###### Compiling Model
model = tf.keras.models.Model(inputs=Input_Layer,outputs=[dense_hgr,dense_id])
model.compile(tf.keras.optimizers.Adam(lr=1e-4),
              loss=['sparse_categorical_crossentropy','sparse_categorical_crossentropy'],
              loss_weights=[0.0,args.lambda_id],
              metrics=['accuracy'])
model.summary()
...
###### Training the Model
history = model.fit(X_train,(y_train,y_train_id),epochs=30,batch_size=8,
                validation_data=(X_dev,(y_dev,y_dev_id)), validation_batch_size=8,

...
gap_op_norm = tf.keras.layers.Lambda(lambda x: tf.math.l2_normalize(x, axis=1))(dense1)
dense2 = tf.keras.layers.Dense(6,activation='softmax')(dense1)

###### Compiling Model
model = tf.keras.models.Model(inputs=Input_Layer,outputs=[dense2,gap_op_norm])
triplet_loss = tfa.losses.TripletSemiHardLoss(margin=args.margin)
model.compile(tf.keras.optimizers.Adam(lr=1e-4),loss=['sparse_categorical_crossentropy',triplet_loss])
model.summary()
#tf.keras.utils.plot_model(model)

##### Defining Callbacks
filepath= './Models/'+args.exp_name+'.h5'
checkpoint = tf.keras.callbacks.ModelCheckpoint(filepath,monitor='val_loss',save_best_only=True,mode='max')

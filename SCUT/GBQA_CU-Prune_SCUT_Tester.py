...
from dataReaderPruneSCUT import *
from tdsNet import *
...
X_train_list, y_train_list, _ = dataset_iterator(directory,
                                    3,
                                    143,
                                    1,
                                    10,
...
X_dev_list, y_dev_list, _ = dataset_iterator(directory,
                                    3,
                                    50,
                                    2,
                                    10,
...
dense2 = tf.keras.layers.Dense(3,activation='softmax')(dense1)
...
g_hgr = model.predict(dev_gen,batch_size=4)
y_preds = np.argmax(g_hgr,axis=-1)

#### Saving Predictions
print(model.evaluate(dev_gen,batch_size=4))
np.savez_compressed('./Predictions/'+args.exp_name+'.npz',y_preds)

###### Confusion Matrix
cm = confusion_matrix(np.array(y_dev_list),y_preds)
...
Test_Embeddings = testing_model.predict(dev_gen)
Train_Embeddings = testing_model.predict(train_gen)
...
np.savez_compressed('./Embeddings/GBQA_tdsNet_CU-Prune-Train_SCUT.npz',Train_Embeddings)
...
for idx,color_index in zip(list(np.arange(6)),["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#8c564b"]):
    plt.scatter(tsne_X_dev[np.array(y_dev_list) == idx, 0],tsne_X_dev[np.array(y_dev_list )== idx, 1],s=55,color=color_index,edgecolors='k',marker='h')
